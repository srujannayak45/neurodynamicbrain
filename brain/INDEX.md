...
- B-179 | @git @workflow @gotcha @meta @sync | machine with multiple gh CLI accounts | gh CLI has ONE active account; switching breaks git push/pull to the other identity's repos — includes the silent/automated variant where an unattended sync hook's best-effort push swallows the same 404 with zero visible error → B-179-gh-cli-single-active-account-gotcha.md
